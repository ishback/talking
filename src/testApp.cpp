...
void testApp::setup() {
    ...
    calibration.setup(w, h, wWin);
    sense.setup(w, h, wWin);
}
...
void testApp::update() {
    ...
    if (movie.isFrameNew()) {
        ...
        calibration.update(movie.getPixels());
        
        if (!calibration.isCalibrated) {
            //        cout << "not calibrated" << endl;
//            return;
        } else {
            sense.update(calibration.getCalibrationProperties(), movie.getPixels());
//            sense.update(mesh, movie.getPixels());
        }
    }
...
void testApp::drawRGB() {
    ofImage rgb;
    rgb.allocate(w, h, OF_IMAGE_COLOR);
    rgb.setFromPixels(movie.getPixels(), w, h, OF_IMAGE_COLOR);
    ofPushMatrix();
    {
        ofTranslate(h, h - 240);
        ofScale(0.25, 0.25);
